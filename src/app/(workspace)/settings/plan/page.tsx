...
export default async function PlanUsagePage() {
  ...
  const meters = [{ label: "Bookings this month", used: usage.booking.used, limit: usage.booking.included },
    ...
    { label: "Owner seats", ...usage.owners }, { label: "Staff seats", ...usage.staff }];
  return <div className="foundation-page starter-plan-page"><Link className="text-link" href="/settings">← Settings</Link><p className="eyebrow">SETTINGS / PLAN & USAGE</p><h1>Plan & Usage</h1><p className="foundation-lead">See what your venue uses today and what’s included in your plan.</p>
    ...
    {usage.booking.plan !== "STARTER" && <section className="foundation-card starter-panel"><h2>Included in your plan</h2><div className="starter-month-grid"><div><span>Online payments</span><strong>Available</strong></div><div><span>Advanced analytics</span><strong>Active</strong></div><div><span>Scheduled reports</span><strong>Active</strong></div>
      {["BUSINESS", "PRO"].includes(usage.booking.plan) && <><div><span>Memberships & packages</span><strong>Available</strong></div><div><span>Dynamic pricing</span><strong>Available</strong></div><div><span>Waitlist & automations</span><strong>Available</strong></div><div><span>WhatsApp integration</span><strong>Provider needed</strong></div></>}</div><p className="starter-muted">A payment provider, delivery channels, and an automation schedule must be configured separately. Included features do not imply those external connections are live.</p></section>}
    ...
    {usage.booking.plan === "PROFESSIONAL" && <section className="starter-premium-preview"><span>NEXT STEP</span><h2>Business · RM179/month</h2><p>3,000 bookings, peak pricing, recurring bookings, memberships, packages, promotions, and customer growth tools.</p><Link className="button button-secondary" href="/settings/plans">Compare plans</Link></section>}
    {usage.booking.plan === "BUSINESS" && <section className="starter-premium-preview"><span>NEXT STEP</span><h2>Pro · RM279/month</h2><p>Planned for multi-branch venues: custom domain, advanced permissions, and API access. Not available yet.</p><Link className="button button-secondary" href="/settings/plans">Compare plans</Link></section>}
  </div>;
}
