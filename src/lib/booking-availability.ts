...
import { bookings, branches, operatingHours, resourceBlocks, resources } from "@/db/schema";
...
export async function loadAvailabilitySnapshot(database: BookingDatabase, organizationId: string, branchId: string, resourceIds: string[], from: Date, to: Date) {
  ...
  const [venue, spaces, hours, blocks, reserved] = await Promise.all([
    database.select().from(branches).where(and(eq(branches.id, branchId), eq(branches.organizationId, organizationId))).limit(1),
  ...
  if (!venue[0] || !venue[0].isActive) throw new BookingError("BRANCH_INACTIVE", "This branch is not active");
  ...
  return { branch: venue[0], spaces, hours, blocks, reserved };
}

export function evaluateAvailability(snapshot: AvailabilitySnapshot, resourceId: string, startAt: Date, endAt: Date, now: Date, excludingBookingId?: string) {
  ...
  if (!duration || duration < space.minimumDurationMinutes || (space.maximumDurationMinutes !== null && duration > space.maximumDurationMinutes) ||
    duration % space.bookingIntervalMinutes !== 0) return { available: false as const, reason: "INVALID_DURATION" };
  if (!advanceAllowed(startAt, now, space.minimumAdvanceMinutes, space.maximumAdvanceDays)) return { available: false as const, reason: "ADVANCE_WINDOW" };
  ...
  if (snapshot.reserved.some(booking => booking.id !== excludingBookingId && booking.resourceId === resourceId && (booking.status !== "AWAITING_PAYMENT" || (booking.holdExpiresAt && booking.holdExpiresAt > now)) && startAt < booking.endAt && endAt > booking.startAt)) return { available: false as const, reason: "CONFLICT" };
...
export async function availableSlotsForResources(database: BookingDatabase, actorId: string, organizationId: string,
  raw: { branchId: string; resourceIds: string[]; localDate: string; durationMinutes?: number }, now = new Date()) {
  if (!await authorizedMembership(database, actorId, organizationId, "booking:view")) throw new BookingError("PERMISSION_DENIED", "Booking access denied");
  const input = z.object({ branchId: z.uuid(), resourceIds: z.array(z.uuid()).min(1).max(100), localDate: dateInput, durationMinutes: z.number().int().positive().max(1440).optional() }).parse(raw);
  ...
  return Object.fromEntries(snapshot.spaces.map(space => {
    ...
    for (const window of windowList) {
      for (let start = window.startAt.getTime(); start + duration * 60_000 <= window.endAt.getTime(); start += space.bookingIntervalMinutes * 60_000) {
        ...
        if (evaluateAvailability(snapshot, space.id, startAt, endAt, now).available) slots.set(startAt.toISOString(), { startAt, endAt });
      }
    }
